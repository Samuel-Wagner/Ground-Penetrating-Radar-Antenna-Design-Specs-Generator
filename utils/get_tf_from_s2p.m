function H = get_tf_from_s2p(filename, f_desired)
    % get_tf_from_s2p
    % gets a voltage transfer function from an s2p file
    %
    % inputs
    % filename - filename 
    ...
    %
    % requires RF toolbox for the sparameters() function.

    S = sparameters(filename);
    f = S.Frequencies;

    % throw input warnings
    ...
    % unset the non-calculate frequencies. this can also be done better with a FIR filter.
